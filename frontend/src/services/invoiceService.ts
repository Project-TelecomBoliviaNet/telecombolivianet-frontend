...
export const invoiceService = {
  ...
  getStatsRange: async (dateFrom: string, dateTo: string): Promise<InvoiceRangeStatsDto> => {
    const res = await api.get<ApiResponse<InvoiceRangeStatsDto>>(
      '/invoices/stats/range', { params: { dateFrom, dateTo } });
    return res.data.data!;
  },
  ...
  // US-26 · Vista previa antes de ejecutar
  getBillingPreview: async (year: number, month: number): Promise<BillingPreviewDto> => {
    const res = await api.get<ApiResponse<BillingPreviewDto>>(
      '/invoices/billing-preview', { params: { year, month } });
    return res.data.data!;
  },
  ...
  // Cobranza · Clientes con facturas pendientes
  getDeudores: async (search?: string, from?: string, to?: string): Promise<DeudorDto[]> => {
    const params: Record<string, string> = {};
    if (search) params.search = search;
    if (from)   params.from   = from;
    if (to)     params.to     = to;
    const res = await api.get<ApiResponse<DeudorDto[]>>('/invoices/deudores', { params });
    return res.data.data!;
  },
