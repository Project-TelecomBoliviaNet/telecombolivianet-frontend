...
import { Search, Loader2, AlertCircle, Save, X, CheckCircle } from 'lucide-react';
...
export function CreateTicketModal({ technicians, onClose, onCreated }: Props) {
  const [form, setForm] = useState({
    ...
    Description: '', SupportGroup: '', SlaDurationHours: '',
    AssignedToUserId: '',
  });
  ...
  const handleSubmit = async () => {
    if (!selectedClient)        { setError('Selecciona un cliente de la lista.'); return; }
    ...
    try {
      await ticketService.create({
        ...
        SlaDurationHours: form.SlaDurationHours ? parseInt(form.SlaDurationHours) : undefined,
      });
      onCreated(); onClose();
    } catch (e: unknown) {
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[92vh] flex flex-col">
        ...
        <div className="p-6 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="label">SLA personalizado (h) <span className="text-gray-400 font-normal">opt.</span></label>
              <input type="number" min={1} className="input-field" placeholder="auto"
                value={form.SlaDurationHours} onChange={e => set('SlaDurationHours', e.target.value)} />
            </div>
          </div>
