...
import {
  ...
  Star, Info, AlertTriangle, User, MessageSquare,
} from 'lucide-react';
import { ticketService } from '@/services/ticketService';
import type { TicketDetailDto, CommentType } from '@/types/ticket.types';
...
export function TicketDetailModal({ ticket, technicians, isAdmin, onClose, onRefresh }: Props) {
  ...
  const [tab, setTab]         = useState<'info' | 'comments' | 'worklogs' | 'visits'>('info');
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[92vh] flex flex-col">
        ...
        <div className="flex items-start justify-between p-5 border-b border-gray-100 shrink-0">
          <div className="flex-1 min-w-0">
            ...
            <h2 className="text-base font-bold text-gray-900 leading-tight">{detail.Subject}</h2>
            <p className="text-xs text-gray-400 mt-0.5">{detail.ClientName} · {detail.ClientTbn}</p>
          </div>
