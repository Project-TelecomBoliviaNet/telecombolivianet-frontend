...
import { z } from 'zod';
import { KeyRound, Loader2, AlertCircle, CheckCircle2, ShieldAlert, ArrowLeft } from 'lucide-react';
      ...
      <div className="w-full max-w-md">
        {isMandatory ? (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-5 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-amber-800">Debes cambiar tu contraseña temporal</p>
              <p className="text-xs text-amber-700 mt-0.5">No puedes acceder al sistema hasta establecer una contraseña personal.</p>
            </div>
          </div>
        ) : (
          <Link to="/dashboard" className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800 mb-5 transition-colors">
        ...
        <div className="card p-6">
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
            <div>
              <label className="label">{isMandatory ? 'Contraseña temporal recibida' : 'Contraseña actual'}</label>
            ...
            <button type="submit" disabled={isLoading} className="btn-primary w-full py-2.5 mt-2">
              {isLoading
                ...
                : isMandatory ? 'Establecer contraseña y continuar' : 'Actualizar contraseña'}
            </button>
          </form>
        </div>
      </div>
