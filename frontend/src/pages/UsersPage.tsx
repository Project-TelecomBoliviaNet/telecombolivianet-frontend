...
import {
  ...
  Eye, EyeOff, ShieldCheck, KeyRound, Mail,
} from 'lucide-react';
import {
  userService,
...
const statusBadge = (s: string) => {
  const map: Record<string, string> = {
    ...
    PendienteActivacion: 'bg-amber-100 text-amber-800 text-xs px-2 py-0.5 rounded-full font-medium',
  };
  const labels: Record<string, string> = {
    Activo:              'Activo',
    Bloqueado:           'Bloqueado',
    Inactivo:            'Inactivo',
    PendienteActivacion: 'Pendiente activación',
  };
                  ...
                  <tr key={u.Id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        ...
                        {/* Reenviar activación */}
                        {u.Status === 'PendienteActivacion' && (
                          <button
                            title="Reenviar email de activación"
                            onClick={() => quickAction(
                              () => userService.resendActivation(u.Id),
                              'Reenviar activación',
                              `¿Reenviar el email de activación a ${u.Email}?`
                            )}
          ...
          <form onSubmit={createForm.handleSubmit(handleCreate)} noValidate className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">
                Opcional. Requerido para técnicos que reciben avisos de asignación.
              </p>
            </div>
