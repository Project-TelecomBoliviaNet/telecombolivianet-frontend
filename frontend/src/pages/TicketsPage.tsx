...
import type {
  TicketListItemDto, TicketDetailDto, TicketFilterRequest, TicketKpiDto, SlaAlertsDto,
...
import {
  ...
  fmtMs, getSlaInfo, priorityBadge, statusBadge, typeBadge,
...
function KpiCards({ kpi }: { kpi: TicketKpiDto }) {
  const totalSla = kpi.SlaCompliantCount + kpi.SlaBreachedCount;
  const slaGlobal = totalSla > 0
    ? Math.round(kpi.SlaCompliantCount / totalSla * 100)
    : null;
  ...
  return (
    <div className="space-y-3 mb-5">
      ...
      {/* Fila 2: SLA global + por prioridad + MTTR/MTTA + CSAT */}
      {(slaGlobal !== null || kpi.AvgResolutionMinutes !== null || kpi.AvgFirstResponseMinutes !== null) && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">

          {/* SLA global */}
          {slaGlobal !== null && (
            <div className="card p-3">
              <p className="text-xs text-gray-500 mb-2">Cumplimiento SLA global</p>
              <div className="flex items-center gap-2 mb-2">
                <div className="flex-1 bg-gray-100 rounded-full h-2">
                  <div
                    className="bg-green-500 h-2 rounded-full transition-all"
                    style={{ width: `${slaGlobal}%` }}
                  />
                </div>
                <span className="text-sm font-bold text-gray-700">{slaGlobal}%</span>
              </div>
              <p className="text-xs text-gray-400">
                {kpi.SlaCompliantCount} cumplidos · {kpi.SlaBreachedCount} incumplidos
              </p>
            </div>
          )}

          {/* SLA por prioridad */}
          {kpi.SlaByPriority?.length > 0 && (
            <div className="card p-3">
              <p className="text-xs text-gray-500 mb-2">SLA por prioridad</p>
              <div className="space-y-1.5">
                {kpi.SlaByPriority.map((p) => {
                  const tot = p.Compliant + p.Breached;
                  const pct = tot > 0 ? Math.round(p.Compliant / tot * 100) : 0;
                  return (
                    <div key={p.Priority} className="flex items-center gap-2">
                      <span className="text-xs text-gray-500 w-12 shrink-0">{p.Priority}</span>
                      <div className="flex-1 bg-gray-100 rounded-full h-1.5">
                        <div
                          className={`${PRIORITY_COLORS[p.Priority] ?? 'bg-gray-400'} h-1.5 rounded-full`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
          ...
          {/* MTTR / MTTA */}
          {(kpi.AvgResolutionMinutes !== null || kpi.AvgFirstResponseMinutes !== null) && (
            <div className="card p-3">
              <p className="text-xs text-gray-500 mb-2">Tiempos promedio</p>
              <div className="space-y-2">
                {kpi.AvgResolutionMinutes !== null && (
                  <div>
                    <p className="text-xs text-gray-400">MTTR (resolución)</p>
                    <p className="text-base font-bold text-gray-800">
                      {fmtMinutes(kpi.AvgResolutionMinutes)}
                    </p>
                  </div>
                )}
                {kpi.AvgFirstResponseMinutes !== null && (
                  <div>
                    <p className="text-xs text-gray-400">MTTA (1ª respuesta)</p>
                    <p className="text-base font-bold text-gray-800">
                      {fmtMinutes(kpi.AvgFirstResponseMinutes)}
                    </p>
                  </div>
                )}
              </div>
          ...
          {kpi.AvgCsatScore !== null && (
            <div className="card p-3 flex flex-col justify-center items-center">
              <p className="text-xs text-gray-500 mb-1">CSAT promedio</p>
              <p className="text-2xl font-bold text-yellow-500 flex items-center gap-1">
                <Star className="w-5 h-5" /> {kpi.AvgCsatScore?.toFixed(1)}
              </p>
              <p className="text-xs text-gray-400 mt-0.5">sobre 5.0</p>
            </div>
          )}
        </div>
      )}
...
export default function TicketsPage() {
  ...
  const setSlaBreachedCount = useTicketStore(s => s.setSlaBreachedCount);
  ...
  const [slaAlerts,  setSlaAlerts]  = useState<SlaAlertsDto | null>(null);
  ...
  const [filter, setFilter] = useState<TicketFilterRequest>({
    Search: '', Status: 'all', Priority: 'all', Type: 'all', PageNumber: 1, PageSize: 20,
  });
  ...
  const loadKanban = useCallback(async () => {
    try {
      setKanban(await ticketService.getKanban({
        ...
        Priority: filter.Priority, Type: filter.Type, AssignedToId: filter.AssignedToId,
      }));
    } catch { /* no crítico */ }
  }, [filter.Search, filter.Status, filter.Priority, filter.Type, filter.AssignedToId]);

  const loadSlaAlerts = useCallback(async () => {
    try {
      const alerts = await ticketService.getSlaAlerts();
      setSlaAlerts(alerts);
      setSlaBreachedCount(alerts.Breached.length);
    } catch { /* no crítico */ }
  }, [setSlaBreachedCount]);
  ...
  useEffect(() => { loadSlaAlerts(); }, [loadSlaAlerts]);
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="card p-4 mb-5">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select className="input-field w-auto" value={filter.Type ?? 'all'}
            onChange={e => setFilter(f => ({ ...f, Type: e.target.value, PageNumber: 1 }))}>
